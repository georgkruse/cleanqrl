...
class Grover_agent(tune.Trainable):
    ...
    def setup(self, config: dict):
        ...
        self.max_grover_steps = int(round(
            np.pi / (4 * np.arcsin(1. / np.sqrt(2 ** self.num_qubits))) - 0.5))
        # quality values
        self.state_vals = np.zeros(self.state_space)
        # grover steps taken
        self.num_grover_steps = np.zeros((self.state_space, self.action_space), dtype=int)
        # boolean flags to signal maximum amplitude amplification reached
        self.grover_steps_flag = np.zeros((self.state_space, self.action_space), dtype=bool)
    ...
    def step(self):
        ...
        for step in range(num_steps):
            ...
            if self.config['mode'] == 'grover':
                probs = self.qnode_actor(state, self.config, self.grover_circuit_operators, self.num_qubits) 
            ...
            self.state_vals[state] += self.config['alpha']*(reward
        ...
        # return trajectories
        return deepcopy({	"steps_trained_total": self.steps_trained_total,
