...
class TSPEnv(gym.Env):
    def __init__(self, config):
        ...
        if "quantum" in config["agent"]:
            ...
            print(
                "If a quantum agent is used, the number of cities is determined by the number of qubits"
            )
        else:
        ...
        self.fully_connected_qubits = list(
            combinations(list(range(self.num_cities)), 2)
        )
        ...
        state_space = {}
        state_space["linear_terms"] = Box(
            -np.inf, np.inf, shape=(self.num_cities, 2), dtype="float64"
        )
        state_space["quadratic_terms"] = Box(
            -np.inf,
            np.inf,
            shape=(len(self.fully_connected_qubits), 3),
            dtype="float64",
        )
        state_space["annotations"] = Box(
            -np.inf, np.inf, shape=(self.num_cities, 2), dtype="float64"
        )
    ...
    def reset(self, seed=None, options=None, use_specific_timestep=False, timestep=0):
        ...
        for edge in self.fully_connected_qubits:
            self.fully_connected_edges.append(
                (self.tsp_graph_nodes[edge[0]], self.tsp_graph_nodes[edge[1]])
            )
            edge_distance = np.linalg.norm(
                np.asarray(self.tsp_graph_nodes[edge[0]])
                - np.asarray(self.tsp_graph_nodes[edge[1]])
            )
        ...
        self.partial_tour = np.stack(
            [
                [
                    int(self.fully_connected_qubits[idx][0]),
                    int(self.fully_connected_qubits[idx][1]),
                    0,
                ]
                for idx in range(len(self.fully_connected_qubits))
            ]
        )

        state["linear_terms"] = np.stack(
            [[int(key), value] for key, value in zip(linear.keys(), linear.values())]
        )
        state["quadratic_terms"] = np.stack(
            [
    ...
    def step(self, action):
        ...
        if len(self.tour) > 1:
            reward = self.compute_reward(
                self.tsp_graph_nodes, self.prev_tour, self.tour
            )
        ...
        next_state["linear_terms"] = np.stack(
            [[int(key), value] for key, value in zip(linear.keys(), linear.values())]
        )
        next_state["quadratic_terms"] = np.stack(
            [
                [int(key[0]), int(key[1]), value]
                for key, value in self.edge_weights.items()
            ]
