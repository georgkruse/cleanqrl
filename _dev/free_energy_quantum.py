...
def stack_dict_values(list_of_dicts):
    ...
    return {
        key: torch.cat([d[key] for d in list_of_dicts if key in d], dim=0)
        for key in all_keys
        if any(key in d for d in list_of_dicts)
    }
...
def execute_dwave(h, J, config, sampler, path, sample_index, embedding_path, layers):
    ...
    if config.return_embedding:
        ...
        embedding_data["embedding"] = dict(
            sample_set.info["embedding_context"]["embedding"]
        )
        embedding_data["chain_strength"] = float(
            sample_set.info["embedding_context"]["chain_strength"]
        )
        embedding_data["chain_break_method"] = str(
            sample_set.info["embedding_context"]["chain_break_method"]
        )
        embedding_data["embedding_parameters"] = dict(
            sample_set.info["embedding_context"]["embedding_parameters"]
        )
...
def free_energy_quantum(config: dict):
    ...
    self.replay_buffer = ReplayBufferQBM(
        size=self.config["replay_buffer_config"]["capacity"]
    )
    ...
    if self.sampler_type in [
        ...
        qpu = DWaveSampler(
            solver=dict(topology__type="pegasus", name="Advantage_system6.4"),
            token=token,
        )
        if not config["fixed_embedding"]:
            self.sampler = EmbeddingComposite(
                ...
                embedding_parameters={
                    "max_no_improvement": config["embedding"]["max_no_improvement"],
                    "random_seed": config["embedding"]["random_seed"],
                },
            )
        else:
            embedding = np.load(
                f"{self.embedding_path}/{self.layers}/{self.hamiltonian_type}/{config['embedding_id']}.npy",
                allow_pickle=True,
            ).item()
    ...
    def generate_ising_model(self, visible_nodes):
        ...
        if "H_(d+1)" in self.hamiltonian_type:
            w_plus = math.log10(
                math.cosh(self.big_gamma * self.beta / self.num_replicas)
                / math.sinh(self.big_gamma * self.beta / self.num_replicas)
            ) / (2 * self.beta)

            J = np.zeros(
                (
                    self.w_hh.shape[0] * self.num_replicas,
                    self.w_hh.shape[1] * self.num_replicas,
                )
            )
            for k in range(self.num_replicas):
                for i in range(self.w_hh.shape[0]):
                    for j in range(self.w_hh.shape[1]):
                        if j > i:
                            J[
                                i + self.w_hh.shape[0] * k, j + self.w_hh.shape[0] * k
                            ] = (self.w_hh[i, j] / self.num_replicas)

            for i in range(self.w_vh.shape[1]):
                for k in range(self.num_replicas - 1):
                    J[
                        i + (k * self.w_vh.shape[1]), i + ((k + 1) * self.w_vh.shape[1])
                    ] = w_plus
            ...
            for k in range(self.num_replicas):
                for i in range(self.w_vh.shape[1]):
                    for j in range(visible_nodes.shape[0]):
                        h[i + self.w_vh.shape[1] * k] += (
                            self.w_vh[j, i] * visible_nodes[j]
                        ) / self.num_replicas
    ...
    def init_weights(self):
        ...
        if len(self.layers) == 1:
            ...
            self.w_hh[upper_indices] = np.random.normal(
                self.mean, self.variance, len(upper_indices[0])
            )
            self.w_vh = np.random.normal(
                self.mean,
                self.variance,
                (self.action_size + self.state_size, self.total_qubits),
            )

        else:
            for qubits_layer_0 in range(self.layers[0]):
                for qubits_layer_1 in range(
                    self.layers[0], self.layers[0] + self.layers[1]
                ):
                    self.w_hh[qubits_layer_0, qubits_layer_1] = np.random.normal(
            ...
            if len(self.layers) == 3:
                for qubits_layer_1 in range(
                    ...
                    for qubits_layer_2 in range(sum(self.layers[:2]), sum(self.layers)):
                        self.w_hh[qubits_layer_1, qubits_layer_2] = np.random.normal(
                            self.mean, self.variance
                        )

            self.w_vh = np.zeros(
                (self.state_size + self.action_size, self.total_qubits)
            )

            for state_qubit in range(self.state_size):
                for qubits_layer_0 in range(self.layers[0]):
                    self.w_vh[state_qubit, qubits_layer_0] = np.random.normal(
                        self.mean, self.variance
                    )
            if len(self.layers) == 2:
                for action_qubit in range(
                    self.state_size, self.state_size + self.action_size
                ):
                    for qubits_layer_1 in range(
                        self.layers[0], self.layers[0] + self.layers[1]
                    ):
                        self.w_vh[action_qubit, qubits_layer_1] = np.random.normal(
                            self.mean, self.variance
                        )
            if len(self.layers) == 3:
                for action_qubit in range(
                    ...
                    for qubits_layer_2 in range(sum(self.layers[:2]), sum(self.layers)):
                        self.w_vh[action_qubit, qubits_layer_2] = np.random.normal(
                            self.mean, self.variance
                        )
    ...
    def update_weights(self, next_state, samples, reward, free_energy, visible_nodes):
        ...
        samples = np.mean(
            np.reshape(samples, (samples.shape[0], self.num_replicas, -1)), axis=1
        )

        for action_next_time_step in self.env.legal_actions(next_state):
            next_state, reward_next_time_step, done, _, infos = self.env.step(
                action_next_time_step, next_state
            )
            ...
            (
                free_energy_next_timestep,
                average_hamiltonian_energy,
                entropy,
                samples_next_time_step,
            ) = self.get_free_energy_and_samples(
                visible_nodes=visible_nodes_next_time_step
            )
            ...
            probabilities[upper_triangle_indices] += np.sum(
                samples[:, upper_triangle_indices[0]]
                * samples[:, upper_triangle_indices[1]]
                / samples.shape[0],
                axis=0,
            )
            probabilities += np.diag(np.sum(samples, axis=0) / samples.shape[0])
            update = self.lr * (
                reward + (self.small_gamma * free_energy_next_timestep - free_energy)
            )

            self.w_hh[upper_triangle_indices] -= (
                update * probabilities[upper_triangle_indices]
            )
            for i in range(self.w_vh.shape[1]):
                for j in range(visible_nodes.shape[0]):
    ...
    def execute_sampler(self, h, J):
        ...
        if self.sampler_type in ["dwave-qpu", "dwave-qpu-SA", "dwave-qpu-SQA"]:
            ...
            embedding_path_layers = (
                config.embedding_path + f"/{layers}/{hamiltonian_type}"
            )
        ...
        elif self.sampler_type in [
            "dwave-sim",
            "dwave-sim-SA",
            "dwave-sim-SQA",
            "dwave-sim-SA-H-eff",
        ]:
            sample_set = self.sampler.sample_ising(
                h=h_dict, J=J_dict, **self.config["simulator_config"]
            )
    ...
    def make_action(self, state):
        ...
        if random.random() < epsilon:
            ...
            free_energy, average_hamiltonian_energy, entropy, samples = (
                self.get_free_energy_and_samples(visible_nodes=visible_nodes)
            )
            data_tuple = (
                free_energy,
                action,
    ...
    def step(self):
        ...
        if "learning_rate_schedule" in self.config.keys():
            self.lr = linear_schedule(
                *self.config["learning_rate_schedule"], self.global_step
            )
            print(self.lr)
        ...
        for step in range(self.steps_per_epoch):
            # print(step)
            free_energy, action, samples, visible_nodes, reward, next_state, done = (
                self.make_action(deepcopy(state))
            )
                ...
                if self.global_step % self.train_frequency == 0:
                    for batch in range(self.train_batch_size):
                        (
                            ...
                            sample_free_energy,
                            sample_samples,
                            sample_visible_nodes,
                        ) = self.replay_buffer.sample(1)
                        # next_state, samples, reward, free_energy, visible_nodes
                        self.update_weights(
                            sample_next_obs,
                            sample_samples,
        ...
        if hasattr(self.config, "gamma_schedule"):
            self.big_gamma = linear_schedule(
                self.config["gamma_schedule"], self.global_step
            )
